...
import developmentpermission.filter.AuthenticationFilter;
...
public class FilterConfig {
	...
	/** 行政のみ許可するパス */
	@Value("${app.filter.goverment}")
	private String govermentJson;
	/** アクセス不能パス */
	...
	public FilterRegistrationBean<AuthenticationFilter> authenticationFilter() {
		...
		List<String> govermentList = new ArrayList<String>();
		...
		try {
			...
			govermentList = objectMapper.readValue(govermentJson, new TypeReference<List<String>>() {
			});
			...
			LOGGER.error("app.filter.goverment: " + govermentJson);
			LOGGER.error("app.filter.unable: " + unableJson);

			ignoreList = new ArrayList<String>();
			govermentList = new ArrayList<String>();
		...
		FilterRegistrationBean<AuthenticationFilter> bean = new FilterRegistrationBean<AuthenticationFilter>(
				new AuthenticationFilter(ignoreList, govermentList, unableList));
