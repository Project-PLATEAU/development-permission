...
function setPermission(){
    ...
    if(decoded["X-ROLE"]){
        terria.setPermission(decoded["X-ROLE"]);
    }
}
...
//現在のtokenをpermissionにセット
setPermission();
//tokenの検証を行う
fetch(apiUrl + "/auth/checkAuth")
.then(res => {
    if(res.status !== 200){
        if(!terria.authorityJudgment()){
            fetch(apiUrl + "/auth/checkAuth?jigyousya=true")
            .then(res => {
                if(res.status === 201){
                    // 権限の設定
                    setPermission();
                    // 利用者規約の表示
                    viewState.showUserAgreementView();
                }else{
