...
export default class UserDrawing extends MappableMixin(
  ...
  constructor(options: Options) {
    ...
    this.freeDrawMode = defaultValue(options.freeDrawMode, false);
  ...
  enterDrawMode() {
    ...
    //カスタム用追加
      ...
      const freeangle = {
        ...
        hierarchy :new CallbackProperty((() => {
          return new PolygonHierarchy(this.terria.activeShapePoints);
        }).bind(this), false),
        material : Color.ORANGE.withAlpha(0.5),
        outline : true,
        outlineColor : Color.BLACK}
      };
      this.otherEntities.entities.add(<any>freeangle);
    } else {
    ...
    this.disposePickedFeatureSubscription = reaction(
      ...
      async (pickedFeatures, reaction) => {
        if (isDefined(pickedFeatures) && !this.freeDrawMode) {
