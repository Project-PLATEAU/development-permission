...
export default class Cesium extends GlobeOrMap {
  ...
  constructor(terriaViewer: TerriaViewer, container: string | HTMLElement) {
    ...
    inputHandler.setInputAction(e => {
      if (!this.isFeaturePickingPaused){
        ...
        if (pickPositionCartographic) {
          ...
          this.terria.setLat(latitude + "");
          this.terria.setLon(longitude + "");
          //地図選択(事業者)
    ...
    inputHandler.setInputAction(
      e => {
          ...
          if (pickPositionCartographic && this.terria.clickMode === "1") {
            ...
            const longitude = CesiumMath.toDegrees(pickPositionCartographic.longitude);
            //地図選択 - 自由形選択(事業者)
            console.log(this.terria.activeShapePoints);
            if(this.terria.activeShapePoints.length < 1){
              userDrawingMode = false;
            }
            if(!userDrawingMode){
              userDrawingMode = true;
              this.terria.initSetActiveShapeLonLat([longitude,latitude]);
              this.terria.initSetActiveShapePoints(pickPosition);
              this.terria.initUserDrawing(userDrawing);
              this.terria.setActiveShapePoints(pickPosition);
    ...
    inputHandler.setInputAction(
      e => {
        const pickRay = this.scene.camera.getPickRay(e.endPosition);
        const pickPosition = this.scene.globe.pick(pickRay, this.scene);
        const pickPositionCartographic = pickPosition && Ellipsoid.WGS84.cartesianToCartographic(pickPosition);
        if (pickPositionCartographic && this.terria.clickMode === "1") {
          let div = document.getElementById("tooltips");
    ...
    inputHandler.setInputAction(
      e => {
        if(userDrawing && this.terria.clickMode === "1"){
          userDrawingMode = false;
          userDrawing.endDrawing();
          this.terria.initActiveShapeLonLat();
          this.terria.initActiveShapePoints();
