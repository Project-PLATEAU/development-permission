...
export default class Terria {
   ...
   * 権限チェック
   * TODO 認証機構決定後ロジック変更
   * @return {boolean} 管理者 true それ以外false
   */
      authorityJudgment(){
   ...
   * @type {Array}
   */
  @observable activeShapePoints:Array<any> = [];

  @action
  setActiveShapePoints(position:any) {
    this.activeShapePoints.push(position);
  }

  @action
  initSetActiveShapePoints(position:any) {
    this.activeShapePoints=[];
    this.activeShapePoints.push(position);
  }

  @action
  initActiveShapePoints() {
    this.activeShapePoints=[];
  }
   ...
   * @type {Array}
   */
   activeShapeLonLat:Array<any> = [];
   userDrawing:any = undefined;

   @action
   setActiveShapeLonLat(lonLat:any) {
     this.activeShapeLonLat.push(lonLat);
   }
 
   @action
   initSetActiveShapeLonLat(lonLat:any) {
     this.activeShapeLonLat=[];
     this.activeShapeLonLat.push(lonLat);
   }

   @action
   initActiveShapeLonLat() {
     this.activeShapeLonLat=[];
   }

   @action
   initUserDrawing(userDrawing:any){
      this.userDrawing = undefined;
      this.userDrawing = userDrawing;
   }
 

  /**
   * clickモード 1：申請地選択（事業者） 2:申請地選択（行政）
   * @type {string}
   */
  @observable clickMode: string = "";
   ...
   * @type {string}
   */
  @observable chibanId: string = "";

  @action
  setChibanId(chibanId: string) {
    this.chibanId = chibanId;
  }
   ...
   * @type {any}
   */
  @observable generalConditionDiagnosis: any = "";

  @action
  setGeneralConditionDiagnosis(generalConditionDiagnosis: any) {
    this.generalConditionDiagnosis = generalConditionDiagnosis;
  }
